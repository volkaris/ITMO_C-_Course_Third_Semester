using Itmo.ObjectOrientedProgramming.Lab4.Commands.FileCommands;
using Itmo.ObjectOrientedProgramming.Lab4.FilePrinters;
using Itmo.ObjectOrientedProgramming.Lab4.ParserExecutionResult;
using Itmo.ObjectOrientedProgramming.Lab4.Parsers.ParsersEnumerator;

namespace Itmo.ObjectOrientedProgramming.Lab4.Parsers.ParserChain.InnerChain.FileInners;

public class FileShowArgumentsBuilder
{
    private IFilePrinter? _printer;
    private string? _mode;
    private string? _pathInCommand;

    public FileShowArgumentsBuilder WithPathInCommand(ParserEnumerator enumerator)
    {
        enumerator.MoveNext();
        _pathInCommand = enumerator.Current;
        return this;
    }

    public FileShowArgumentsBuilder WithPrinter(IFilePrinter printer)
    {
        _printer = printer;
        return this;
    }

    public FileShowArgumentsBuilder WithMode(ParserEnumerator enumerator)
    {
        enumerator.MoveNext();

        _mode = enumerator.Current;
        return this;
    }

    public ParsingResult Build()
    {
        if (_printer is null)
            return new ParsingResult.UnsuccessParsing("Why you did this? For what reason?");

        if (_pathInCommand is null) return new ParsingResult.UnsuccessParsing("Why you did this? For what reason?");

        if (_mode is null) _mode = "console";

        if (_mode is not "console") return new ParsingResult.UnsuccessParsing("mode must be console");

        return new ParsingResult.SuccessParsing(new FileShow(_printer, _pathInCommand));
    }
}